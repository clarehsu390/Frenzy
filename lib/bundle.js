...
const Trie = __webpack_require__(5);

class Game {
  constructor() {
    jQuery.get('https://raw.githubusercontent.com/clarehsu390/Frenzy/master/assets/dictionary.txt', (data) => this.dictionary(data));
  ...
  handleClick() {
    ...
    $(".square").mouseup(function(){
      ...
      if (word.length > 3 &&
        trie.contains(word) && !wordArr.includes(word)) {
        wordArr.push(word);
        score += (5 + word.length - 3);
        $(".score").text(`${score}`);
        $("#feedback").append('<li>Super!</li>');
        $(".submitted").append(`<li>${word}</li>`);
        board.replace();
      }
      if (!trie.contains(word) || word.length <= 2 || wordArr.includes(word)) {
