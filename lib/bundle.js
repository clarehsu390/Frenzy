...
class Game {
  constructor() {
    jQuery.get('https://raw.githubusercontent.com/clarehsu390/WordFrenzy/master/assets/dictionary.txt', (data) => this.dictionary(data));
  ...
  handleClick() {
    ...
    $(".square").mouseup(function(){
      ...
      if (trie.contains(word.toLowerCase()) && word.length >= 3 && !wordArr.includes(word)) {
        wordArr.push(word);
        score += 5;
        $(".score").text(`${score}`);
      ...
      if (word.length > 3 &&
        trie.contains(word.toLowerCase()) && !wordArr.includes(word)) {
        wordArr.push(word);
        score += (word.length - 3);
        $(".score").text(`${score}`);
